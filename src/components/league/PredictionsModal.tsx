...
interface PredictionsModalProps {
  ...
  ongoingGameWeek: LeagueGameWeek;
}

const PredictionsModal = ({ onClose, gameId, league, ongoingGameWeek }: PredictionsModalProps) => {
  ...
  const handleSaveCorrectedPredictions = async () => {
    ...
    const updatedStandings = league.standings.map((standing) => {
      const points = pointsDistributions.find((d) => d.participantId === standing.userId)?.points;
      const predictedCorrectResult = pointsDistributions.find((d) => d.participantId === standing.userId)?.points?.correctResult;
      return {
        ...standing,
        points: points ? points.total : standing.points,
